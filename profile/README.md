# Tzamun Arabia IT Co.

**Leading Technology Partner Delivering Innovative Digital Solutions Across Saudi Arabia and the GCC.**

Tzamun is a Saudi-based technology company specializing in building modern, scalable, and mission‑critical digital platforms.  
We help organizations transform their operations through custom software, cloud services, AI solutions, and enterprise‑grade infrastructure.

---

## 🚀 Our Mission
To become the first choice for clients seeking advanced IT services by delivering custom business software, smart mobile applications, and digital solutions that accelerate growth and operational excellence.

---

## 🧩 What We Do

### **Digital Solutions**
- Custom business software  
- Smart mobile applications (iOS & Android)  
- E‑commerce platforms  
- Corporate and enterprise web applications  
- UI/UX design, prototyping, and user research  

### **Cloud & Infrastructure**
- Cloud hosting (AWS, Azure, Google Cloud)  
- VPS, dedicated hosting, and managed services  
- On‑premise and hybrid cloud deployments  
- Enterprise‑grade AI infrastructure (sovereign, on‑prem, PDPL‑compliant)  

### **Consulting & Support**
- Digital transformation consulting  
- Long‑term technical support  
- Infrastructure optimization  
- System integration and workflow automation  

---

## 🏥 Industry Platforms & Products

### **Healthcare & Medical Platforms**
- **eTabeb** — Appointment booking, offers, promotions, and multi‑profile patient management  
- **PathologyHub** — Connects medical facilities with expert pathologists across KSA  
- **RadiologyHub** — Remote radiology reporting and diagnostic imaging analysis  

### **Enterprise & Operations**
- **TzamunERP** — Full‑scale operations management for healthcare, manufacturing, and SMEs (ERPNext‑based, ZATCA‑compliant)  
- **PAM Solution** — Privileged Access Management for modern data centers, developed and hosted in Saudi Arabia  
- **Infra Command Center** — Unified gateway for API management, access control, and infrastructure monitoring  

### **AI & Developer Tools**
- **TzamunAI** — Enterprise‑grade on‑prem AI with zero vendor dependency  
- **DevAssist (VSCode MCP)** — Context‑aware AI assistant for developers  
- **VoiceHub** — Arabic voice automation for Saudi and GCC markets  

### **Other Digital Ventures**
- **MotorMind** — Automotive workshop and garage management system  
- **Luxury Rental Platform** — Rent premium designer handbags in Saudi Arabia  
- **White‑label AI & Voice Solutions** — In partnership with DataQueue Inc.  

---

## 🛠️ Technology Expertise
- TypeScript, JavaScript, Node.js  
- Python, AI/ML frameworks  
- Docker, Kubernetes  
- ERPNext, Frappe  
- Cloud: AWS, Azure, Google Cloud  
- macOS & Linux infrastructure  
- API gateways, microservices, DevOps  

---

## 🤝 Our Approach
We build long‑term partnerships by:
- Understanding client needs deeply  
- Delivering scalable, future‑proof solutions  
- Providing continuous support and optimization  
- Ensuring security, compliance, and reliability  

---

## 👥 Leadership
Tzamun is led by experienced professionals in digital healthcare, enterprise IT, financial operations, and large‑scale system implementation across Saudi Arabia.

---

## 🌍 Contact Us
**Website:** https://tzamun.sa  
**Email:** info@tzamun.sa  
**Phone:** +966 12 9200 23452  
**Location:** Saudi Arabia  

---

**Innovative technology solutions driving digital transformation and business growth across Saudi Arabia and the GCC.**
